    ...
    public function action() {
        ...
        if ($dao->adminEstConnecte()) {
            ...
            if (isset($_GET['numreservation']) && strlen($_GET['numreservation']) != 0) {
                ...
                $dao = new MysqlDao();
                $tab = $dao->getPassagersEtPlacesByNumresa($_GET['numreservation']);

                // Cas d'un identifiant non valide
                // Si qqn touche à l'URL par exemple :
                if ($dao->getReservationById($_GET['numreservation'])->getNumReservation() == null) {
                    // ci-dessus la méthode "getReservationById" renvoie un objet Reservation créé en fonction
